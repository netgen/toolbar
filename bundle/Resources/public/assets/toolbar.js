const d="components",a="items",c="active";class u{constructor(t){if(this.el=t,this.toggleButtons=this.el.querySelectorAll(".js-toggle-mode"),this.editableComponentBlocks=document.querySelectorAll("[data-component]"),this.editableItemBlocks=document.querySelectorAll("[data-item]"),this.adminUrlTemplate=this.el.dataset.adminUrlTemplate,!this.adminUrlTemplate)throw new Error("Admin edit url is undefined");this.init()}init(){this.toggleButtons.forEach(t=>{t.addEventListener("click",()=>this.toggleViewMode(t))})}getEditableBlocks(t){switch(t){case a:return this.editableItemBlocks;case d:return this.editableComponentBlocks;default:throw new Error("Block type is not one of: "+[a,d].join(" "))}}isButtonActive(t){return[...t.classList].includes(c)}toggleButtonIcon(t){const i=t.querySelector(".js-toggle-mode-icon");if(!i)return;const e=this.isButtonActive(t)?t.dataset.iconHideSrc:t.dataset.iconShowSrc;e&&(i.src=e)}toggleButtonLabel(t){const i=t.querySelector(".js-toggle-mode-label");if(!i)return;const e=this.isButtonActive(t)?t.dataset.labelHide:t.dataset.labelShow;e&&(i.innerHTML=e)}toggleButtonActive(t){t.classList.toggle(c)}removeEditButtonIfExists(t){const i=t.querySelector(".js-edit-button"),e=t.querySelector(".js-edit-outline");!i||!e||(i.remove(),e.remove(),t.dataset.initialPosition&&(t.style.position=t.dataset.initialPosition))}addEditButtonIfDoesntExist(t){const i=t.querySelector(".js-edit-button"),e=t.querySelector(".js-edit-outline");if(!(i&&e))try{const o=this.formatEditButtonUrl(t);t.insertAdjacentHTML("beforeend",this.editButtonMarkup(o)),t.style.position&&t.setAttribute("data-inital-position",t.style.position),t.style.position="relative"}catch(o){console.warn(o)}}toggleViewMode(t){this.toggleButtonActive(t),this.toggleButtonIcon(t),this.toggleButtonLabel(t),this.getEditableBlocks(t.dataset.blockType).forEach(e=>{e.classList.toggle("js-editing-enabled"),this.removeEditButtonIfExists(e),this.addEditButtonIfDoesntExist(e)})}get editButtonStyles(){return`
  ...
  `}formatEditButtonUrl(t){const{contentId:i,locationId:e}=t.dataset;if(!this.adminUrlTemplate)throw new Error("Admin url template is undefined");if(!i&&!e)throw new Error(`Neither location id nor content id are set on ${t}`);let o=[];const r=this.adminUrlTemplate.includes("{contentId}"),l=this.adminUrlTemplate.includes("{locationId}");if(r&&!i&&o.push("Content id is not defined."),l&&!e&&o.push("Location id is not defined."),o.length>0)throw Error(o.join(`
`));let n=this.adminUrlTemplate;return r&&(n=n.replace("{contentId}",i)),l&&(n=n.replace("{locationId}",e)),n}}document.addEventListener("DOMContentLoaded",()=>{const s=document.querySelector("#ngtoolbar");s&&new u(s)});
